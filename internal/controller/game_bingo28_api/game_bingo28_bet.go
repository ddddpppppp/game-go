...
func (c *ControllerV1) Bingo28Bet(ctx context.Context, req *v1.Bingo28BetReq) (res *v1.GameCommonRes, err error) {

	message := fmt.Sprintf("bet %s, amount $%.2f", req.BetType, req.BetAmount)
	err = game_bingo28_ws.BroadcastToAllUsers(ctx, g.Map{
		"action": "new_message",
		"data": g.Map{
			"id":         "",
