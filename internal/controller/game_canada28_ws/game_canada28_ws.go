...
func (c *WsController) Connect(r *ghttp.Request) {
	...
	// 升级连接并存储
	ws, _ := r.WebSocket()
	conn := game_canada28_ws.StoreConnection(userID, ws)

	readChan := make(chan []byte)
	errorChan := make(chan error)

	// 独立读取协程
	go func(conn *game_canada28_ws.Connection) {
		for {
			select {
			case <-conn.Ctx.Done():
				return
	...
	for {
		...
		case err := <-errorChan:
			game_canada28_ws.SafeCloseConnection(conn) // 关闭当前实例
			handleCloseError(ctx, err, userID)
			return
		case <-ctx.Done():
			game_canada28_ws.SafeCloseConnection(conn)
			return
		}
	}
}

func processMessage(ctx context.Context, conn *game_canada28_ws.Connection, data []byte) error {
	var msg Message
