...
type SyncBingo28ResCron struct {
	ctx context.Context
}

func NewSyncBingo28ResCron(ctx context.Context) *SyncBingo28ResCron {
	return &SyncBingo28ResCron{
		ctx: ctx,
	}
}

// 1天未完成的直接设为失败
func (c *SyncBingo28ResCron) DoSync() error {
	var draw *do.Bingo28Draws
	err := dao.Bingo28Draws.Ctx(c.ctx).Where("status in (0,1) and end_at < ?", gtime.Now()).Order("id desc").Scan(&draw)
	if err != nil {
		return err
	}
	if draw == nil {
	...
	// 当前时间
	var redisKey = fmt.Sprintf(consts_sync.SyncBingo28ResKey, draw.Id)
	...
	dao.Bingo28Draws.Ctx(c.ctx).Where("draw_id=?", draw.Id).Update("status=1")
	...
	// 主API地址
	primaryAPI := "https://apigx.cn/token/3caa44a49e8611f09d7d279a02232d9c/code/twbg28/rows/3.json"
	// 备用API地址
	backupAPI := "https://vip.apigx.cn:2096/token/3caa44a49e8611f09d7d279a02232d9c/code/twbg28/rows/3.json"
	...
	if useBackupAPI {
		...
		if resMap28["data"] == nil {
			g.Log().Error(c.ctx, g.Map{
				"error":         "sync bingo28 primary API error",
	...
	for _, item := range list {
		if gconv.String(item["expect"]) == gconv.String(draw.PeriodNumber) {
			...
			draw.Status = 2
			_, err = dao.Bingo28Draws.Ctx(c.ctx).Where("id=?", draw.Id).Update(draw)
			if err != nil {
				g.Log().Error(c.ctx, g.Map{
					"error":         "update bingo28 draws error",
					"msg":           err.Error(),
					"period_number": draw.PeriodNumber,
				})
				return err
			...
			_, err = dao.Bingo28Draws.Ctx(c.ctx).Insert(g.Map{
			...
			if err != nil {
				g.Log().Error(c.ctx, g.Map{
					"error":         "insert bingo28 draws error",
					"msg":           err.Error(),
					"period_number": draw.PeriodNumber,
				})
				return err
...
// 计算动态赔率
func (c *SyncBingo28ResCron) calculateDynamicOdds(betType string, originalOdds float64, resultSum int, merchantId string) float64 {
	// 查询适用的动态赔率规则
	var rules []*do.Bingo28DynamicOdds
	err := dao.Bingo28DynamicOdds.Ctx(c.ctx).
...
// 检查规则条件是否满足
func (c *SyncBingo28ResCron) checkRuleCondition(rule *do.Bingo28DynamicOdds, resultSum int) bool {
...
func (c *SyncBingo28ResCron) bonus(draw *do.Bingo28Draws) {
	// 获取该期所有等待开奖的投注记录
	var redisKey = fmt.Sprintf(consts_sync.SyncBingo28BonusKey, draw.Id)
	res, err := g.Redis().Get(c.ctx, redisKey)
	if err != nil {
		return
	}
	...
	for _, bet := range bets {
		...
		if isWin {
			...
			balanceUpdates = append(balanceUpdates, userBalanceUpdate{
				...
				description: fmt.Sprintf("Bingo28 Win - Period:%s, Bet Type:%s, Multiplier:%.2f (Original:%.2f)", draw.PeriodNumber, betName, finalMultiplier, multiplier),
				relatedId:   betId,
			...
			if frozenWinAmount > 0 {
				...
				frozenBalanceUpdates = append(frozenBalanceUpdates, userFrozenBalanceUpdate{
					...
					description: fmt.Sprintf("Bingo28 Win - Period:%s, Bet Type:%s, Multiplier:%.2f (Original:%.2f) [Frozen Balance]", draw.PeriodNumber, betName, finalMultiplier, multiplier),
					relatedId:   betId,
	...
	err = g.DB().Transaction(c.ctx, func(ctx context.Context, tx gdb.TX) error {
		...
		for _, update := range betUpdates {
			...
			_, err := tx.Model("game_bingo28_bets").
		...
		dao.Bingo28Draws.Ctx(ctx).Where("id=?", draw.Id).Update("status=3")
	...
	if err != nil {
		g.Log().Warning(c.ctx, g.Map{
			"error": "send bingo28 res msg to all users error",
			"msg":   err.Error(),
		})
	}
...
func (c *SyncBingo28ResCron) drawResult(draw *do.Bingo28Draws) {
	err := game_bingo28_ws.BroadcastToAllUsers(c.ctx, g.Map{
		"action": "draw_result",
		"data": g.Map{
			"period_number":  draw.PeriodNumber,
